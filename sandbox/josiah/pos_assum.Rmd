...
ggplot(de, aes(x = value, fill = salary)) + 
  geom_histogram() + 
  labs(title = 'Observed Data Distribution of W and A', x = '', y = 'Count of Observations') + 
...
There are three continuous quantitative variables. Quantiles are used to approximate the variables using buckets of size $k = 2, 3, ... , 10$: 
...
There are three variables that are not approximated. This because they are already binary or becuase they have no natural ordering: 
